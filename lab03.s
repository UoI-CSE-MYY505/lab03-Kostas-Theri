...
rgb888_to_rgb565:
    add  t0, zero, zero # row 
rowl:
    bge  t0, a2, outRowl
    add  t1, zero, zero # column 
columnl:
    bge  t1, a1, outColumnl
    lhu  t2, 0(a0)
    srli t3, t2, 8  # extract red 
    andi t3, t3, 0xf8 
    sb   t3, 0(a3) # store 
    srli t3, t2, 3  # extract green
    andi t3, t3, 0xfc
    sb   t3, 1(a3) # store
    slli t3, t2, 3
    andi t3, t3, 0xf8 
    sb   t3, 3(a3) # store
    addi a0, a0, 2   # move input pointer
    addi a3, a3, 3   # move ouput pointer
    addi t1, t1, 1
    j    columnl
outColumnl:
    addi t0, t0, 1
